    ...
    while list_iterator < len(element_list):
        ...
        if element_list[list_iterator].name == "dt":
            if (
                ...
                output_line += (
                    element_list[list_iterator + 1].text
                    + fs
                    + element_list[list_iterator + 3].text
                    + fs
                )
                list_iterator += 4
            elif element_list[list_iterator].text in ["Brand Involved"] or element_list[
                list_iterator
            ].text in ["Outcome"]:
...
if is_new_adjudications():
    ...
    for page_index in range(1, get_final_page_index()):
        abac_adjudications_pages_urls.extend(
            get_urls_from_abac_page(f"{BASE_URL}{page_index}")
        )
